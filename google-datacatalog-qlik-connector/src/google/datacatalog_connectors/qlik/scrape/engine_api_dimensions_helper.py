...
class EngineAPIDimensionsHelper(base_engine_api_helper.BaseEngineAPIHelper):
    ...
    async def __receive_get_dimensions_msg(self, websocket, responses_manager):
        return await self._receive_messages(websocket, responses_manager,
                                            self.__GET_PROPERTIES,
                                            'result.qProp')

    async def __send_get_dimensions_msg(self, websocket, responses_manager):
    ...
    async def __send_follow_up_msg_get_dimensions(self, websocket,
        ...
        if responses_manager.is_method(response_id, self._OPEN_DOC):
            ...
            await self.__handle_get_dimension_reply(websocket,
                                                    responses_manager,
                                                    response)
            responses_manager.remove_unhandled(response)
    ...
    async def __handle_get_dimension_reply(self, websocket, responses_manager,
                                           response):
